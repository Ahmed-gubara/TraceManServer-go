...
const FCS_START uint16 = 0xffff
const FCS_FINAL uint16 = 0xf0b8
...
/****************************************************************************
* Functional Description: CRC control c
ode
...
func GetFcs(fcs uint16, src uint8) uint16 {
	var xor uint16 = 0
	var iresult uint16 = 0
	xor = fcs
	xor ^= uint16(src)
...
*Functional desc
ription: CRC check
* input parameters: psrc: to check data
ilen: data length
* output parameters: None
...
1: failed
****************************************************************************/
func CRC_CheckCrc(psrc []uint8) bool {
	var crc uint16 = 0
	crc = FCS_START
	for _, i8 := range psrc {
		crc = GetFcs(crc, i8)
...
****************************************************************************/
func CRC_MakeCrc(psrc []uint8) uint16 {
	var crc uint16 = 0
	crc = FCS_START
	for _, i8 := range psrc {
		crc = GetFcs(crc, i8)
